...
messageRouter.get('/:userId/conversations', getUserId, async(req: Request, res) => {
  try {
      ...
      .select((eb) => [
        ...
        jsonArrayFrom(
          eb
            ...
            .innerJoin('picture as p', 'p.user_id', 'u.id')
            .select(['u.id', 'u.name as receiver_name', 'p.picture_path', 'u.birthdate as receiver_birthdate'])
